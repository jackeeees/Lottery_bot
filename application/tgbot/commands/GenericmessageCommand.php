    ...
    public function execute()
    {
        ...
        if ( ($type == 'group' || $type == 'supergroup') && empty($text) == false ){
            ...
            foreach ($LotteryList as $index => $Lottery){
                // 查询抽奖口令
                if ($is_bot == false && (
                        strpos ( $text ,  'lottery' ) !== false ||
                        strpos ( $text ,  'Lottery' ) !== false ||
                        strpos ( $text ,  'LOTTERY' ) !== false ||
                        strpos ( $text ,  'sweepstake' ) !== false ||
                        strpos ( $text ,  'raffle' ) !== false ||
                        strpos ( $text ,  'LOTTERy' ) !== false ) ){
                    if ($Lottery->join_type == 1){  // 参与方式 1:群聊
                        $keyword_text .= ($index+1) . ". Send『<b>{$Lottery->keyword}</b>』to participate in <b>{$Lottery->title}</b> lottery；" . PHP_EOL . PHP_EOL;
                        ...
                        $keyword_text .= ($index+1) . ". [<a href=\"{$join_link}\">Click here</a>] to participate in <b>{$Lottery->title}</b> lottery；" . PHP_EOL . PHP_EOL;
                    }
                }
                ...
                if ($is_bot == false && strpos ( $text ,  $Lottery->keyword ) !== false){
                    ...
                    if ( !$bot_info ){
                        ...
                        if ( $bot_info['ok'] ){
                            ...
                            $data = [
                                ...
                                'text'    => 'Bot information verification failed',
                            ];
                    ...
                    if ($bot_info['result']['status'] != 'administrator'){
                        $data = [
                            'chat_id' => $chat_id,
                            'reply_to_message_id' => $message_id,
                            'text'    => 'Bot is not administrator, please contact the group administrator!',
                        ];
                        Queue::push('app\tgbot\job\AutoSendMessage', [
                            'method' => 'sendMessage',
                            'data' => $data,
                    ...
                    if ($type == 'supergroup' && ($bot_info['result']['can_delete_messages'] == false || $bot_info['result']['can_pin_messages'] == false)){
                        $data = [
                            'chat_id' => $chat_id,
                            'reply_to_message_id' => $message_id,
                            'text'    => 'The bot permission setting is incorrect, please contact the group administrator!',
                        ];
                        Queue::push('app\tgbot\job\AutoSendMessage', [
                            'method' => 'sendMessage',
                            'data' => $data,
                    ...
                    switch ( $Lottery->conditions ){
                        case  1 :    // 按时间自动开奖
                            $condition_text = 'Release time of prize:' . $Lottery->condition_time;
                            break;
                        case  2 :   // 按人数自动开奖
                            $condition_text = "Release of prizes when the number of participants reaches <b>{$Lottery->condition_hot}</b> ";
                            break;
                    }
                    ...
                    if ( !$LotteryUser ){
                        ...
                        $data = [
                            'chat_id' => $chat_id,
                            'reply_to_message_id' => $message_id,
                            'text'    => 'Thank you for your participation <b>' . $Lottery->title . '</b> lottery' . PHP_EOL .
                                'Number of prizes:<b>' . $Lottery->number . '</b>' . PHP_EOL .
                                $condition_text . PHP_EOL .
                                'Current Participants:<b>' . ($Lottery->hot + 1) . '</b>' . PHP_EOL . PHP_EOL .

                                'Please start a private chat with the bot @' . $bot_username . ' ,If you win, the robot will send you the prize via a private chat.' . PHP_EOL .
                                'Do not leave the group during the lottery, as this will disqualify you from winning.' . PHP_EOL .
                        ...
                        $data = [
                            'chat_id' => $chat_id,
                            'reply_to_message_id' => $message_id,
                            'text'    => "You have participated in the <b>{$Lottery->title}</b> lottery" . PHP_EOL .
                                $condition_text . PHP_EOL .
                                'Current number of participants:<b>' . $Lottery->hot . '</b>',
                            'parse_mode' => 'html',
                            'disable_web_page_preview' => true,
                        ];
                    }
